...
factory.extend({
  _constructor: function() {
    ...
    this.set('server', connect().use(connect.bodyParser()));
  ...
  use: function(middleware) {
    this.getProperty('server').use(middleware);
  },
  ...
  listen: function(port) {
    ...
    plugins.forEach(function(x) {
      self.getProperty('server').use(x);
    });

    this.getProperty('server').listen(port || 1337);
  },
